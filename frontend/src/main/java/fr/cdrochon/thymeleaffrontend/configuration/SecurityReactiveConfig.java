...
public class SecurityReactiveConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http,
        ...
        return http
                ...
                .oauth2Login(oauth -> oauth.userInfoEndpoint(userInfo -> userInfo.oidcUserService(keycloakOidcUserService)))
    ...
    @Bean
    public OAuth2UserService<OidcUserRequest, OidcUser> keycloakOidcUserService() {
        OidcUserService delegate = new OidcUserService();
        return userRequest -> {
            OidcUser oidcUser = delegate.loadUser(userRequest);
            Set<GrantedAuthority> mappedAuthorities = new HashSet<>(oidcUser.getAuthorities());

            extractRealmRoles(oidcUser).forEach(role -> mappedAuthorities.add(new SimpleGrantedAuthority("ROLE_" + role)));
            extractClientRoles(oidcUser, keycloakClientId).forEach(role -> mappedAuthorities.add(new SimpleGrantedAuthority("ROLE_" + role)));
    ...
    private static Set<String> extractRealmRoles(OidcUser oidcUser) {
        Object realmAccess = oidcUser.getClaims().get("realm_access");
        if (!(realmAccess instanceof Map<?, ?> realmMap)) {
            return Set.of();
        }
        return normalizeRoles(realmMap.get("roles"));
    }

    private static Set<String> extractClientRoles(OidcUser oidcUser, String clientId) {
        Object resourceAccess = oidcUser.getClaims().get("resource_access");
        ...
        return normalizeRoles(clientMap.get("roles"));
    }

    private static Set<String> normalizeRoles(Object rolesObject) {

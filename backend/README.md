...
Fichiers importants
- `backend/.env.example` : exemple de variables d'environnement pour le dev local. Copier en `backend/.env` (ne jamais committer `.env`).
...
- `backend/dev-run.sh` : wrapper pour compatibilité (redirige vers `scripts/dev-env.sh`).
...
```bash
chmod +x scripts/dev.sh scripts/dev-up.sh scripts/dev-env.sh backend/dev-run.sh
```
...
- Pour l'onboarding, fournissez soit :
  ...
  - une procédure pour obtenir les secrets auprès de l'équipe Ops (ticket, email sécurisé), ou
  - un script sécurisé `scripts/generate-local-env.sh` pour générer des credentials locaux (inclus dans le repo si souhaité).
